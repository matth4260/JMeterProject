...
docker run --name jmeterkey -v "$2":/SharedVolume docker-registry-default.app.leanovia.net/jmeter/jmeterkey
docker rm jmeterkey
...
do
	...
	docker run --name jmeterserv$i --network=JMeter_Network -d -v "$2":/SharedVolume docker-registry-default.app.leanovia.net/jmeter/jmeterserv
...
docker run --name jmetercont --network=JMeter_Network -it -v "$2":/SharedVolume -e NUMBER_OF_INJECTORS=$1 docker-registry-default.app.leanovia.net/jmeter/jmetercont
